...
echo
printf 'y\n' | aibox init my-project --yes --base debian --profile human-dev --user aibox --theme gruvbox --prompt default --tmux-status extended --addon python --addon-tool python:python=3.13 --addon-tool python:uv=0.11.14 --harness claude --processkit-version v0.26.15 --no-container 2>&1 || true
