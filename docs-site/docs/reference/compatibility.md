...
|--------------|-----------------|-------|
| 0.23.9 | v0.25.5 | restores shell-backed Zellij status rows as the default, hardens `aibox-status` against `/proc` races, fixes Yazi edit actions, and applies addon dependency fallback handling to `aibox up` |
