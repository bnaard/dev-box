...
use crate::config::{AiHarness, AiboxConfig, BaseImage, McpGatewayMode};
use crate::output;
...
const IMAGE_TAG_RUNTIME_CUTOFF_MINOR: u64 = 27;

fn dockerfile_image_tag(base: &BaseImage, version: &str) -> String {
    match version {
        "latest" if current_cli_uses_runtime_image_tag() => {
            format!("base-{}-runtime-latest", base)
        }
        "latest" => format!("base-{}-latest", base),
        _ if should_use_runtime_image_tag(version) => format!("base-{}-runtime-v{}", base, version),
        _ => format!("base-{}-v{}", base, version),
    }
}

fn current_cli_uses_runtime_image_tag() -> bool {
    should_use_runtime_image_tag(env!("CARGO_PKG_VERSION"))
}

fn should_use_runtime_image_tag(version: &str) -> bool {
    let Ok(parsed) = semver::Version::parse(version) else {
        return false;
    };

    parsed.major > 0 || parsed.minor >= IMAGE_TAG_RUNTIME_CUTOFF_MINOR
}
...
fn generate_dockerfile(
    ...
    let content = tmpl
        .render(context! {
            ...
            image_tag => dockerfile_image_tag(&config.container.image.base, &config.container.image.version),
...
mod tests {
    ...
    fn dockerfile_contains_correct_from_image() {
        ...
        let image_tag = dockerfile_image_tag(
    ...
    #[test]
    fn dockerfile_uses_legacy_tag_family_for_0_26_series() {
        let dir = tempfile::tempdir().unwrap();
        let mut config = make_config(&["python"], false);
        config.container.image.version = "0.26.9".to_string();
        config.image.version = config.container.image.version.clone();
        generate_dockerfile(&config, dir.path(), &test_env()).unwrap();
        let content = fs::read_to_string(dir.path().join("Dockerfile")).unwrap();

        assert!(
            content.contains("FROM ghcr.io/projectious-work/aibox:base-debian-v0.26.9 AS aibox"),
            "0.26.x projects must continue using legacy base image tag family"
        );
    }

    #[test]
    fn image_tag_prefers_runtime_family_post_cutover() {
        assert_eq!(
            dockerfile_image_tag(&BaseImage::Debian, "1.2.3"),
            "base-debian-runtime-v1.2.3"
        );
        assert_eq!(
            dockerfile_image_tag(&BaseImage::Debian, "0.27.0"),
            "base-debian-runtime-v0.27.0"
        );
        assert_eq!(
            dockerfile_image_tag(&BaseImage::Debian, "0.26.3"),
            "base-debian-v0.26.3"
        );
    }

    #[test]
    fn latest_tag_family_follows_current_cli_minor_cutover() {
        let expected = if current_cli_uses_runtime_image_tag() {
            "base-debian-runtime-latest"
        } else {
            "base-debian-latest"
        };

        assert_eq!(dockerfile_image_tag(&BaseImage::Debian, "latest"), expected);
    }
    ...
    fn dockerfile_installs_tau_in_shared_uv_tool_directory() {
        ...
        let content = fs::read_to_string(dir.path().join("Dockerfile")).unwrap();
        assert!(
            content.contains("UV_TOOL_DIR=/opt/aibox/uv-tools uv tool install tau-ai==0.3.10"),
