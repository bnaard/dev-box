...
pub(crate) fn build_substitution_map(config: &AiboxConfig) -> HashMap<&'static str, String> {
    ...
    //    Re-evaluated on every `aibox apply` (because render runs at
    ...
    // 10. AI_PROVIDERS — sorted, comma-separated effective AI harnesses.
...
pub fn scaffold_context(config: &AiboxConfig) -> Result<()> {
    ...
    write_if_missing(
        ...
        "# Docker Compose override — project-specific services and overrides.\n\
         # This file is never overwritten by `aibox apply`. You own it.\n\
         #\n\
         # Docker Compose automatically merges this with the generated\n\
         # docker-compose.yml (strategic merge by service name).\n\
         # When present, `aibox apply` wires it into devcontainer.json.\n\
         #\n\
...
/// enabled AI harnesses that have a markdown convention. Today only Claude
...
pub(crate) fn update_gitignore(addons: &AddonsSection) -> Result<()> {
    ...
    // Generated from .aibox-local.toml by `aibox apply`; contains credentials
    ...
    // Regenerated by `aibox apply` for every developer from their own config.
...
fn ensure_aibox_entries(gitignore_path: &Path) -> Result<()> {
    let required_entries = [
        ...
        ".aibox-local.toml",
        // Generated from .aibox-local.toml by `aibox apply` — contains the same
...
mod tests {
    ...
    fn update_gitignore_creates_when_missing() {
        in_temp_dir(|| {
            ...
            assert!(body.contains(".devcontainer/Dockerfile"));
            assert!(body.contains(".devcontainer/docker-compose.yml"));
