...
#
# Renders a single PowerKit segment for the proc metric (process/thread count).
...
plugin_get_metadata() {
    ...
    metadata_set "description" "aibox proc metric: process/thread count from the aibox-status helper"
}
...
plugin_render() {
    printf '%s/%s' "$(plugin_data_get processes)" "$(plugin_data_get threads)"
}
