...
tmux -f "$config" new-session -d -s "$session" -n ai -c "$workspace" "$(tool_or_shell yazi)"
files_pane="$(tmux display-message -p -t "$session:ai" '#{pane_id}')"
agent_pane="$(tmux split-window -t "$session:ai" -h -p 50 -P -F '#{pane_id}' -c "$workspace" "$(tool_or_shell codex)")"
