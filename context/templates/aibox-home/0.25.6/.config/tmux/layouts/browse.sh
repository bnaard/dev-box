...
tmux -S "$socket" -f "$config" new-session -d -s "$session" -n browse -c "$workspace" "$(tool_or_shell yazi)"
files_pane="$(tmux -S "$socket" display-message -p -t "$session:browse" '#{pane_id}')"
agent_pane="$(tmux -S "$socket" split-window -t "$session:browse" -v -p 35 -P -F '#{pane_id}' -c "$workspace" "$(tool_or_shell claude)")"
tmux -S "$socket" select-pane -t "$files_pane"
tmux -S "$socket" select-window -t "$session:browse" 2>/dev/null || true
exec tmux -S "$socket" -f "$config" attach-session -t "$session"
