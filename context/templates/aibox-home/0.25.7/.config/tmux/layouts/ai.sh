...
agent_pane="$(tmux -S "$socket" split-window -t "$session:ai" "$split_flag" -p "$split_ratio" -P -F '#{pane_id}' -c "$workspace" "$(tool_or_shell codex)")"
agent_pane_2="$(tmux -S "$socket" split-window -t "${agent_pane}" -v -p 20 -P -F '#{pane_id}' -c "$workspace" "$(tool_or_shell claude)")"
