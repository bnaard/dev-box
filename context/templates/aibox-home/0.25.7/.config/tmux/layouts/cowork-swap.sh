...
tmux -S "$socket" -f "$config" new-session -d -s "$session" -n cowork-swap -c "$workspace" "$(tool_or_shell yazi)"
...
agent_pane="$(tmux -S "$socket" split-window -t "$session:cowork-swap" "$split_flag" -p "$split_ratio" -P -F '#{pane_id}' -c "$workspace" "$(tool_or_shell claude)")"
...
tmux -S "$socket" select-window -t "$session:cowork-swap" 2>/dev/null || true
exec tmux -S "$socket" -f "$config" attach-session -t "$session"
