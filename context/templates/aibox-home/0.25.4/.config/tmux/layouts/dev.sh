...
tmux -S "$socket" -f "$config" new-session -d -s "$session" -n dev -c "$workspace" "$(tool_or_shell vim)"
...
agent_pane="$(tmux -S "$socket" split-window -t "$session:dev" -v -p 40 -P -F '#{pane_id}' -c "$workspace" "$(tool_or_shell codex)")"
