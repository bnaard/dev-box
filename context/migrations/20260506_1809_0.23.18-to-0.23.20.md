...
**Status:** pending
**aibox CLI:** v0.23.18 → v0.23.20
**processkit:** v0.25.8

## Summary

aibox has been updated from v0.23.18 to v0.23.20. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.18 and v0.23.20 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.23.20 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.20` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.20"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.20.
