...
**Status:** completed (verified 2026-05-03: project has advanced to aibox 0.23.0 / processkit v0.25.0; processkit migration index has no pending or in-progress entries; `aibox apply --no-container` reports no migration needed)
...
- [ ] Verify `/workspace/context/templates/processkit/v0.18.2/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.18.2/AGENTS.md context/templates/processkit/v0.18.2/AGENTS.md`
...
processkit is tracking `version = "latest"` — installed: `v0.18.2` (upgraded from `v0.18.2` — aibox.toml now targets `vlatest`) (source: `https://github.com/projectious-work/processkit.git`).
...
- [ ] processkit version reference matches `aibox.lock` (`v0.18.2`)
