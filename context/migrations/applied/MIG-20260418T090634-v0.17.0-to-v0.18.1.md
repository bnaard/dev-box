...
kind: Migration
metadata:
  id: MIG-20260418T090634
  created: 2026-04-18 09:06:34+00:00
spec:
  source: processkit
  ...
  generated_at: 2026-04-18 09:06:34+00:00
  applied_at: 2026-04-18 11:30:00+00:00
...
- All 8 conflicts resolved as no-ops: 4 skill-gate scripts had been pre-patched in the prior session and now match upstream v0.18.1 byte-for-byte; `context/schemas/{actor,role}.yaml` and `context/skills/INDEX.md` were already at v0.18.1; `AGENTS.md` intentionally retains the local v2 compliance-contract markers (handover open_thread #3 — upstream itself ships an inconsistency between AGENTS.md template v2 markers and skill-gate v1 markers, to be reconciled when `skip_decision_record` MCP tool ships).
- All 49 new-upstream files were already on disk after `aibox sync` ran 2026-04-18T09:06:34Z. No copy work needed.
- All 23 removed-upstream files were deleted from `context/` in this session.
- Most new + removed pairs are slash-command renames (`*-action.md` → `pk-*.md`); net new content of substance is the `team-creator` skill (already present), owner-profiling assets, and per-skill `config/settings.toml` for id-management and index-management.
- Net behaviour change for agents: new `pk-*` slash commands available; old `*-action` commands removed; team-creator workflow ready.
