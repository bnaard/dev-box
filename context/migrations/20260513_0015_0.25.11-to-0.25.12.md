...
**Status:** pending
**aibox CLI:** v0.25.11 → v0.25.12
**processkit:** v0.26.2

## Summary

aibox has been updated from v0.25.11 to v0.25.12. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.25.11 and v0.25.12 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.25.12 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.25.12` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.25.12"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.25.12.
