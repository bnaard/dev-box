# Migration: v0.28.3 → v1.0.0-alpha.1
...
**Status:** pending
**aibox CLI:** v0.28.3 → v1.0.0-alpha.1
**processkit:** v0.28.1

## Summary

aibox has been updated from v0.28.3 to v1.0.0-alpha.1. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Review the release notes for each version in this upgrade:
- https://github.com/projectious-work/aibox/releases/tag/v1.0.0-alpha.1

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v1.0.0-alpha.1 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `1.0.0-alpha.1` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "1.0.0-alpha.1"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v1.0.0-alpha.1.
