...
**Status:** pending
**aibox CLI:** v0.24.2 → v0.24.3
**processkit:** v0.25.8

## Summary

aibox has been updated from v0.24.2 to v0.24.3. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.24.2 and v0.24.3 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.24.3 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.24.3` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.24.3"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.24.3.
