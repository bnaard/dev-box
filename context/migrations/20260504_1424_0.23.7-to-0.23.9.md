...
**Status:** pending
**aibox CLI:** v0.23.7 → v0.23.9
**processkit:** v0.25.5

## Summary

aibox has been updated from v0.23.7 to v0.23.9. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.7 and v0.23.9 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- **v0.23.9** (processkit v0.25.5): Patch release: restores the shell-backed Zellij status rows as the default, hardens aibox-status against /proc races, fixes Yazi's edit action by using Zellij's editor action instead of injecting Vim commands, and applies addon dependency fallback handling to `aibox up`.
...
- [ ] Owner: verify `aibox apply` was run for v0.23.9 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.9` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.9"`
...
processkit is pinned to `v0.25.5` (source: `https://github.com/projectious-work/processkit.git`).
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.9.
