...
**Status:** completed
**aibox CLI:** v0.23.0 → v0.23.1
**processkit:** v0.25.1

## Summary

aibox has been updated from v0.23.0 to v0.23.1. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.0 and v0.23.1 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.23.1 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.1` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.1"`
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.25.0/AGENTS.md context/templates/processkit/v0.25.1/AGENTS.md`
...
processkit is pinned to `v0.25.1` (upgraded from `v0.25.0` — aibox.toml now targets `v0.25.1`) (source: `https://github.com/projectious-work/processkit.git`).
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.1.
