    ...
    if not settings_path.is_file():
        ...
        if settings is None:
            ...
            if (
                missing_perms
                and in_gateway_mode
                and "mcp__processkit-gateway__*" in live_perms
            ):
                # In gateway mode, the gateway wildcard authorizes the
                # proxied processkit tool surface. Per-skill wildcards may be
                # absent by design.
                missing_perms = []
            if missing_perms:
