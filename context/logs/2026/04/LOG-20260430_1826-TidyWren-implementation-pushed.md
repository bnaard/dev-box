...
spec:
  ...
  details:
    ...
    - reports selected backends, addon availability, container CLI status, MCP registration
      targets, and permission targets
    - keeps canonical processkit provider-backend Artifact emission gated on upstream
      schema
    validation:
    ...
    - AIBOX_ADDONS_DIR=/workspace/addons cli/target/debug/aibox describe provider-backends
      -o json
