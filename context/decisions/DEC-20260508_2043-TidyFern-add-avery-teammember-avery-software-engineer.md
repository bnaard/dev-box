...
spec:
  ...
  rationale: "Adding a single TeamMember surgically (rather than running pk-team-create to bootstrap the full 8-archetype team) is the minimum-disruption fix: it leaves Cora's existing setup intact, fills the engineering gap, and keeps the team small and human-readable. Naming \"Avery\" was suggested by the AI-agent name pool. The personality emphasises code-first, small reversible steps, and refactoring discipline \u2014 fits a generalist senior IC. The role ROLE-20260422_0001-MigratedRole-software-engineer already existed in the project's role catalog, so no role creation was needed."
  alternatives:
