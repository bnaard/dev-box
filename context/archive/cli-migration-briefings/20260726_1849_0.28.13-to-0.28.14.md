...
**Status:** pending
**aibox CLI:** v0.28.13 → v0.28.14
**processkit:** v0.28.3

## Summary

aibox has been updated from v0.28.13 to v0.28.14. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.28.13 and v0.28.14 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.28.14 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.28.14` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.28.14"`
...
- [ ] Verify `/workspace/context/templates/processkit/v0.28.3/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.28.3/AGENTS.md context/templates/processkit/v0.28.3/AGENTS.md`
...
processkit is tracking `version = "latest"` — installed: `v0.28.3` (upgraded from `v0.28.3` — aibox.toml now targets `vlatest`) (source: `https://github.com/projectious-work/processkit.git`).
...
- [ ] processkit version reference matches `aibox.lock` (`v0.28.3`)
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.28.14.
