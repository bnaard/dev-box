...
**Status:** pending
**aibox CLI:** v0.28.2 → v0.28.3
**processkit:** v0.27.5

## Summary

aibox has been updated from v0.28.2 to v0.28.3. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.28.2 and v0.28.3 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.28.3 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.28.3` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.28.3"`
...
- [ ] Verify `/workspace/context/templates/processkit/v0.27.5/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.27.5/AGENTS.md context/templates/processkit/v0.27.5/AGENTS.md`
...
processkit is tracking `version = "latest"` — installed: `v0.27.5` (upgraded from `v0.27.5` — aibox.toml now targets `vlatest`) (source: `https://github.com/projectious-work/processkit.git`).
...
- [ ] processkit version reference matches `aibox.lock` (`v0.27.5`)
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.28.3.
