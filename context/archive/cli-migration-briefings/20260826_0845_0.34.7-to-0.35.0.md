...
**Status:** pending
**aibox CLI:** v0.34.7 → v0.35.0
**processkit:** v0.28.8

## Summary

aibox has been updated from v0.34.7 to v0.35.0. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.34.7 and v0.35.0 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.35.0** (processkit v0.28.8): Minor release: adds Borland, Norton, and Phosphor terminal theme families plus opt-in D2, Graphviz, Vega, Vega-Lite, and Mermaid build-time graphics addons.
  <https://github.com/projectious-work/aibox/releases/tag/v0.35.0>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.35.0 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.35.0` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.35.0"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.35.0.
