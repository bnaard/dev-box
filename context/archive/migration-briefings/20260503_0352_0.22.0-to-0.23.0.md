...
**Status:** completed (verified 2026-05-03: aibox.lock is at CLI 0.23.0 and processkit v0.25.0; processkit migration index has no pending or in-progress entries; `aibox apply --no-container` reports runtime and processkit content are in sync)
**aibox CLI:** v0.22.0 → v0.23.0
**processkit:** v0.25.0

## Summary

aibox has been updated from v0.22.0 to v0.23.0. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.22.0 and v0.23.0 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.23.0 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.0` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.0"`
...
- [ ] Verify `/workspace/context/templates/processkit/v0.25.0/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.25.0/AGENTS.md context/templates/processkit/v0.25.0/AGENTS.md`
...
processkit is pinned to `v0.25.0` (source: `https://github.com/projectious-work/processkit.git`).
...
- [ ] processkit version reference matches `aibox.lock` (`v0.25.0`)
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.0.
