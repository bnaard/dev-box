...
**Status:** pending
**aibox CLI:** v0.23.20 → v0.23.21
**processkit:** v0.25.8

## Summary

aibox has been updated from v0.23.20 to v0.23.21. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.20 and v0.23.21 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Owner: verify `aibox apply` was run for v0.23.21 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.21` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.21"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.21.
