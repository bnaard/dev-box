...
**Status:** completed (verified 2026-05-03: aibox.lock is at CLI 0.23.2 and processkit v0.25.1; aibox.toml pins processkit v0.25.1; AGENTS.md is non-empty; processkit v0.25.1 template snapshot exists; processkit migration index has no pending or in-progress entries after applying MIG-RUNTIME-20260503T110834)
**aibox CLI:** v0.23.1 → v0.23.2
**processkit:** v0.25.1

## Summary

aibox has been updated from v0.23.1 to v0.23.2. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.1 and v0.23.2 is listed below.                  Review each; treat entries tagged as breaking as action items.
...
- [ ] Verify `/workspace/context/templates/processkit/v0.25.1/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.25.1/AGENTS.md context/templates/processkit/v0.25.1/AGENTS.md`
...
processkit is pinned to `v0.25.1` (source: `https://github.com/projectious-work/processkit.git`).
...
- [ ] processkit version reference matches `aibox.lock` (`v0.25.1`)
